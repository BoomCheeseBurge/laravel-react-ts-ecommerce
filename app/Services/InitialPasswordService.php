...
class InitialPasswordService implements PasswordBrokerContract
{
    /**
     ...
     * @var string
     */
    const INITIAL_LINK_SENT = 'passwords.setup';

    /**
     * Constant representing a successfully initial password.
     *
     * @var string
     */
    const PASSWORD_INITIAL = 'passwords.initial';
     ...
     * @return void
     */
    public function __construct(#[\SensitiveParameter] PasswordBroker $tokens, ?Dispatcher $dispatcher = null)
    {
        $this->users = new EloquentUserProvider(app('hash'), new User());
        $this->tokens = $tokens->getRepository();
    ...
    public function sendResetLink(#[\SensitiveParameter] array $credentials, ?Closure $callback = null)
    {
        ...
        $user->notify(new sendInitialPasswordNotification($token));
    ...
    public function reset(#[\SensitiveParameter] array $credentials, Closure $callback)
    {
        ...
        return static::PASSWORD_INITIAL;
    }
    ...
    {
        if (is_null($user = User::where('email', $credentials['email'])->first())) {
            return static::INVALID_USER;
        }
